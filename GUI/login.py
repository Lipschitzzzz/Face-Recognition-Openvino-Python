...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def face_get(self):
        ...
        user_log = person_reidentification.face_recognition()
        date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        # print(user_log)

        if len(user_log) == 0 or user_log == False:
            return 0

        QMessageBox.information(self, 'Info', 'Welcome! ' + user_log[2], QMessageBox.Ok)
        database.database.update_user_log(date, user_log[0], user_log[1], 1)
