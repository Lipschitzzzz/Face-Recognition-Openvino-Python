...
def find_person_id_by_name(person_name):
    ...
    return values[0][0]
def update_user_log(date, confidence, person_id, face_id):
    db = mysql.connector.connect(
        host="localhost",
        user="root",
        passwd="root",
    ...
    cursor = db.cursor()
    sql = "insert into user_log(date, confidence, person_id, face_id) values (%s, %s, %s, %s)"
    value = ([date, confidence, person_id, face_id])
    cursor.execute(sql, value)
    db.commit()
def load():
    db = mysql.connector.connect(
